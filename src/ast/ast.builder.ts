...
export function generateAst(document: OpenAPIV3.Document): AbstractSyntaxTree {
  const declarations: AstNodeDeclaration[] = document.components?.schemas ? generateDeclarations(document.components.schemas) : [];
  const operations: AstNodeOperation[] = document.paths ? generateOperations(document.paths) : [];
...
function generateDeclarations(schemaRecord: Record<string, OpenAPIV3.SchemaObject | OpenAPIV3.ReferenceObject>): AstNodeDeclaration[] {
  const declarations: AstNodeDeclaration[] = [];

  const records = Object.entries(schemaRecord);
  for (const record of records) {
    ...
    nodeType.modifiers = {};
    declarations.push(new AstNodeDeclaration(name, nodeType, modifiers, refName));
  }
  ...
  if (schema.allOf) {
    ...
    if (schema.properties) {
      ...
      for (const propEntry of propEntries) {
        ...
        fields.push(new AstNodeDeclaration(identifier, propertyType, modifiers));
      }
    }
    ...
    return new AstNodeTypePrimative(schema.type, modifiers);
  }
...
function generateOperations(pathObject: OpenAPIV3.PathsObject): AstNodeOperation[] {
  ...
  for (const path of paths) {
    ...
    if (pathItem) {
      nodeOperations.push(...generateOperationFromPathItem(pathPattern, pathItem));
    }
  }
...
function generateOperationFromPathItem(pathPattern: string, pathItem: OpenAPIV3.PathItemObject): AstNodeOperation[] {
  const nodeOperations: AstNodeOperation[] = [];
  ...
  for (const method in OpenAPIV3.HttpMethods) {
    ...
    const responses = generateResponse(operationObject.responses);
    ...
    const requestBody = operationObject.requestBody ? generateRequestBody(operationObject.requestBody) : undefined;

    const request = new AstNodeTypeRequest(requestBody, pathNode, cookieNode, headerNode, queryNode, { description, title });

    const nodeOperation = new AstNodeOperation(
      (operationObject.operationId ?? 'OperationId_NOTDEFINED').replace(/-./g, x => x[1].toUpperCase()),
      method as AstNodeOperationHttpMethod,
      pathPattern,
      responses,
      request,
...
type ParameterLocations = 'query' | 'header' | 'path' | 'cookie' | 'unknown';
...
function generateParameter(parameter: OpenAPIV3.ReferenceObject | OpenAPIV3.ParameterObject): {
  ...
  if (IsReferenceObject(parameter)) {
    return { location: 'unknown', type: generateTypeFromSchema(parameter) };
  }
  ...
  const declaration = new AstNodeDeclaration(name, nodeType, {});
...
function generateResponse(responses: OpenAPIV3.ResponsesObject): AstNodeType | undefined {
...
function generateRequestBody(request: OpenAPIV3.RequestBodyObject | OpenAPIV3.ReferenceObject): AstNodeType {
  if (IsReferenceObject(request)) {
    return generateTypeFromSchema(request);
  }
